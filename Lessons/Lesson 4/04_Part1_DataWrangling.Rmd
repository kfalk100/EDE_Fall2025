...
#Poll
Chain multiple operations on a data frame - pipe
Subset/rearrange columns in dataframe - select (columns)
To calculate new values from existing ones - mutate
To subset rows based on criteria - filter (rows)
Sort rows based on value - arrange
Vignette - documentation

#Shortcuts
Alt and - gives <-
Shift control M gives %>%
Shift alt I gives new line 
...
> Add notes here: They affect growth of plants, fish, and other aquatic species. 
...
dplyr vs tidyverse
  - dplyr is tidy version of plyr
  - tidyverse imports number of packages, including dplyr and ggplot, one that allows us to do pipes
...
select() changes whether a column is included
rename() changes name of column
mutate() changes values of columns and creates new columns
...
# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #filtering rows with 0 column depth, (data set, what you are trying to filter)
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) #depth 0-.25 is surface
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #either paul or peter lake filtering
NTL.phys.data.PeterPaul4 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # ! to exclude lake

# %in% to make it so you do not need to type out lake names
...
#integer
NTL.phys.data.year4 <- filter(NTL.phys.data, year4 == 1999)
# OR
NTL.phys.data.year4 <- NTL.phys.data %>% filter(year4 == 1999)
...
# OR
NTL.phys.data.Tuesday <- NTL.phys.data %>% 
  filter(
    lakename == "Tuesday Lake",
    year4 %in% c(1990:1999)
  )

# comma and & work the same here for filtering - adding another filter
# THIS | is OR

## Use pipes to wrangle data

NTL.phys.data.TuesdayJuly <- NTL.phys.data %>% 
  filter(
    lakename == "Tuesday Lake",
      year4 %in% c(1990:1999),
      month(sampledate) ==7)

# Use summarize function from the above to find mean surface area

summary(NTL.phys.data.TuesdayJuly)
```
Filter then summaries
  Debugging pipe - select UP TO first pipe, run that. If that works, do it again until error is given. 

Question: Why don't we filter using row numbers?

> Answer: Row numbers change all the time. ALSO - Not what we are looking for, columns organize the values
...
NTL.phys.data.descending <- arrange(NTL.phys.data, desc(`temperature_C`))
# Which dates, lakes, and depths have the highest temperatures?
# 1998, 2002, 1995
# East Long Lake and Hummingbird lake
# 0.5 and 0.0
```
### Select

Selecting allows us to choose certain columns (variables) in our dataset. IN RESPECT TO COLUMNS
Colon - range of columns
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) # Same thing
```
...
NTL.phys.data.processed <- 
  ...
  mutate(temperature_F = (temperature_C*9/5) + 32) #one step instead of three
