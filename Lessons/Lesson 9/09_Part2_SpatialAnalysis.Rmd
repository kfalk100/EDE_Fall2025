...
epa_pm25_sites_sf <- read_csv(here('ENV 872/EDE_Fall2025/Data/Raw/EPAair_PM25_NC2018_raw.csv')) %>% 
...
counties_sf<- st_read(here('ENV 872/EDE_Fall2025/Data/Spatial/cb_2018_us_county_20m.shp')) %>% 
...
huc8_sf <- st_read(here('ENV 872/EDE_Fall2025/Data/Spatial/NCHUC8.shp'))
...
huc8_sf2 <- st_read(here('ENV 872/EDE_Fall2025/Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == "Upper Neuse")
mymap2 = mapview(huc8_sf2,
                 col.regions = "yellow",
...
mapview(counties_sf_join, 
        zcol = 'E_POV150', # what you are coloring by
        col.regions = brewer.pal(2, 'RdBu')) + # specifies palette
...
HUC_12sf <- read_csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv")
HUC_12sf$HUC_12 <- as.factor(HUC_12sf$HUC_12)
HUC_12sf <- HUC_12sf %>% 
  mutate(HUC_12sf, HUC_8 = substr())
  group_by(HUC_12)
...
epa_sf_utm <-      st_transform(epa_pm25_sites_sf, crs = 26917) #EPSG.io
...
neuse_intersect <- counties_sf_utm %>%  
  st_filter(neuse_sf, .predicate = st_intersects) #disjoint is opposite of select, google sf predicates

#View the result
myMap + mapview(neuse_intersect, alpha.regions = 0, color='red') # VIEW TWO MAPS
...
HUC8Select <- counties_sf_utm %>% 
  st_filter(triCo, .predicate = st_intersects)

mymap3 + mapview(HUC8Select, color="red")
#Intersect the HUC_8s
HUC8Intersect <- triCo %>% 
  st_intersection(counties_sf_utm)

mymap3 + mapview(HUC8Intersect, alpha.regions = 0)
...
theSite_sfc <- st_sfc(theSite_sfp, crs = 4326) # specify point reference system
...
#Use st_buffer instead of st_intersects
