...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # Split column up to isolate month - space Jan space average. Spaces are null.
  mutate( Date = my(paste0(Month,"-",Year))) %>%  # my() function - month year
...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # specifies that null is a space
...
BR_complete <- inner_join(Rainfall_BR_processed,Temp_BR_processed) # combine two datasets, inner join to keep all observations
...
Months <- as.data.frame(seq.Date(from=as.Date("1901/01/01"),
                                 ...
                                 by="month"))
colnames(Months) <- "Date" # want columns with same name to use join function
...
BRTemp_plot <- ggplot(BR_complete, aes(x=Date, y=Temperature_C)) +
  geom_line() +
  geom_smooth(method=lm, se=FALSE, color="red") +
  labs(x = "Year", y = "Temperature (C)")
plot(BRTemp_plot) # seasonality, increasing temperature over time

BRRainfall_plot <- ggplot(BR_complete, aes(x=Date, y=Rainfall)) +
  geom_line() +
  geom_smooth(method=lm, se=FALSE, color="red")
  labs(x="Year", y="Rainfall (mm)")
plot(BRRainfall_plot) # clear defined seasonality, wet and dry season, flat trend over time, rainfall may go up in some regions but not others
...
BR_Temp_ts <- ts(BR_complete$Temperature_C, start = c(1901,1), frequency = 12) #ymd
...
BR_Rainfall_Trend_Map <- ggplot(BR_Rainfall_Components) +
  geom_line(aes(y = Observed, x = Date), linewidth = 0.25) + 
  geom_line(aes(y = trend, x = Date), color = "red") + 
  geom_hline(yintercept = 0, lty = 2) +
  ylab("Rainfall (mm)")
print(BR_Rainfall_Trend_Map)

BR_Temp_Trend_Map <- ggplot(BR_Temp_Components) +
  geom_line(aes(y = Observed, x = Date), linewidth = 0.25) + 
  geom_line(aes(y = trend, x = Date), color = "red") + 
  geom_hline(yintercept = 0, lty = 2) +
  ylab("Temperature (C)")
print(BR_Temp_Trend_Map)
