...
EPAaircorrelation <- 
  ...
  na.omit()
EPAair.corr <- cor(EPAaircorrelation)
corrplot(EPAair.corr, method = "ellipse") # Color and shape gives strength and sign of correlation 
# corrplot.mixed(EPAair.corr, upper = "ellipse") # Mix of upper and ellipse
...
EPAairAIC <- lm(data = EPAair.omit, PM2.5 ~ Ozone + Year + Month + mean_Lat + mean_Lng)
print(EPAairAIC)

step(EPAairAIC) 
EPAairModel <- lm(data = EPAair.omit, PM2.5 ~ Ozone + Year + Month + mean_Lat + mean_Lng)
summary(EPAairModel)
