...
Increasing/decreasing trend in response variable over time?
Can we forecast conditions in the future?

Plan for future activities/operations

Components:
Time frame - short, medium, long
Seasonal variation - up and down movement w/ short variation
  - Climate, weather condition, custom traditions or habits
Random variations
  - Erratic movements that are not predictable, do not follow a pattern

Terms:
Stationary data- time series variable w. no significant upward movement
  - Opposite is nonstationary data

Covariance and correlation measure joint variability
  - Measure of linear dependence between two variables

Autocorrelation - ACF, measure of dependence between two adjacent values of same variables
  - Give information about dependence structure of time series
  
Stationary process - probability laws that govern the behavior of the process do not change over time

White noise series - sequence of independent, identically distributed random variables

Partial autocorrelation - PACF, linear dependency of variables 
  - Remove influence of intermediate variables - PACF

PACF and ACF always the same at leg 1
  - Changes with each following
  
Plotting ACF - seasonality of data
...
Can have seasonal component and trend
  - Up and down over seasons but increasing over time
  
Removing seasonal trend
  - Run regression and obtain residuals
  - Residuals give detrended series
  
Mann-Kendall Test
  - Employed to detect deterministic trends in series of env data, climate data, hydro data
  - Cannot be applied to seasonal data
  - Checks magnitude of S and its significance, based on # of observations
  - More observations, higher S needs to be to be statistically significant
  - MannKendall(x)
  
Spearman's Rank Correlation Coefficient
  - Statistical measure of strength in monotonic relationship
  - Relationship does not need to be linear like above
  - Compute spearman correlation between data and time series
  
Dick-Fuller Test
  - Yt = alpha + Yt-1 + Et
    - Et - unknown. white noise series
...
* **Heteroscedasticity**: The variance of the time series is not constant over time
  - Difference between T and T-1 is not alwasys the same
...
Can use time series to fill gap between data periods, or just use one half.
...
```
x - univariate - time series - only two variables
s.window = periodic, or span that is odd and >7
...
plot(EnoDischarge.late_Decomposed) # grey bars on end show impact of each component in overall value
...
wind_data_ts <- ts(wind_data_clean$AWND.clean, # ts function on cleaned data
                   start=c(f_year,f_month), # first month and first year of first observation in column date
                   frequency=12) 
...
# null - data is stationary, reject since less than 0.05, there is a trend

wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2) # shows trend per season
...
> Answer: Decreasing trend in average windspeed over time, not stationary except for seasons 4 and 5
...
ARMA models require stationary data. This means that there is no monotonic trend over time and there is also equal variance and covariance across the time series. The function `adf.test` will determine whether our data are stationary. The null hypothesis is that the data are not stationary, so we infer that the data are stationary if the p-value is < 0.05.
  Use SMK to break down each season, Seasonal Mann Kendall for whole data set
