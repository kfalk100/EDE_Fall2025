...
library(ggridges) #variable over time
...
* geom_text # add text or improve labels
...
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) #first two lines just store in a plot, next prints plot
print(O3plot)
...
print(PMplot.faceted) #creates one plot for each site, keeps different shape for different years. removes color for each site name, as they are different plots (one variable per plot)
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # separates into different years 
print(PMplot.faceted2) # pick plots you are interested in, points are no longer different shapes per year, are now on separate plots
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line()
print(PMplot.line) # connect points, need to change x axis to date for all observations. only for one site
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) # does not show much - two outliers, makes the rest of the data appear concentrated. need to adjust axis 
...
lightvsDOfixed <- 
  ...
  xlim(0, 250) +
  ylim(0, 20)
print(lightvsDOfixed) # limit max values in order to get closer look 
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # do not specify, values printed from 0 up. for depth, need 0 to be at top
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) + # categorize by depth
  geom_point() +
  geom_smooth(method = lm) + # to find trend line - LM - linear model
  geom_abline(aes(slope = 16, intercept = 0)) # knew slope and intercept beforehand - can add this way
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - color is regards to contour of shape, fill only adds color to boxplot, and not points
print(Nutrientplot3)

# Dot plot
Nutrientplot4 <- # shows more about data than boxplot with these distributions
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, # adding color in and out of dot, stacking observations over y (binaxis), binwidth - width of bin stack, stackdir - center stack, position = dodge do not overlay dots
...
print(Nutrientplot7)
# adds legend to coloration, unlike previous example which just has tn_ug on x axis and no legend
...
# Ridgeline plot
Nutrientplot6 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(y = nutrient, x = concentration)) + # y is nutrient to put length of graph on y axis
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # # fill used to make lake name area under the plot, different color for each lake
print(Nutrientplot6)
