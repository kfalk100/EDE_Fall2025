...
print(O3plot) # grey with white gridlines

O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray()
print(O3plot1) # defualt gg plot theme

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw()
print(O3plot2) # grid line and axes
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme # can set theme in definition of plot
print(O3plot5)  

theme_set(mytheme) # or you can set here in chunk
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + # lty - dadhed line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) +
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # hjust = 1 means left align # hjust is horizontal justification of text / margin
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), # limits of x axis, gaps of two months between points
    date_breaks = "2 months", date_labels = "%b %y") + # month then year, want month to be abbreviated format (Sep), %y gives year as two digits
  ylab(expression("O"[3]* " AQI Value")) + #change y label to be more informative, [3] gives superscript
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) # angle x axis labels for points
print(O3plot7)  
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + # adding transparency to points, chaging size
  labs(color = "Depth", shape="Lake name") + # change your legend labels  to look better
  scale_shape_manual(values = c(15, 17)) + # changing shapes to bet more different between Peter and Paul Lake
  #scale_color_distiller(palette = "Blues", direction = 1) + # use scale_color_brewer for discrete variables
  scale_color_viridis(option = "magma", direction = -1) +
  theme(legend.position = "right", # putting plot on right side
        legend.text = element_text(size = 12), legend.title = element_text(size = 12)) # changing text size
print(NvsP2)
...
# putting two plots together in same window
...
ggplot(Neonics) +
  ...
  reorder(Neonics$Endpoint,Neonics)
