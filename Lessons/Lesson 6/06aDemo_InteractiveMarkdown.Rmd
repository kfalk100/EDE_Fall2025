...
my_plot <- nutrient_data %>% 
  filter(year4 > 1990 & year4 < 2016) %>% #filtering years 1990>year>2016
  ggplot(aes(
    x = sampledate, 
    y = tp_ug,
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) + # changing transparency, size
  scale_x_date(
    date_breaks = "1 years", # showing every year
    date_labels = '%Y') + #4 digits
  scale_shape_manual(values = c(17, 15)) + # 17 triangle, 15 square
  labs(
  ...
  scale_color_viridis_c(
    option = "mako", 
    guide = guide_colorbar(reverse = T), # changes gradient of legend
    direction = -1) # changes gradient of plot points
...
#Construct the plot
my_plot <- nutrient_data %>% #change variables/parameters to terms
  filter(year4 > start_year & year4 < end_year) %>% 
  ggplot(aes(
    x = sampledate, 
    y = !!sym(the_parameter), #changing to "the_parameter" does not work, condenses values to one line. Looking for column called "the_parameter" - has no values. Use two bangs (!!) and sym() to make work
    color = depth,
    shape = lakename)) +
...
``` 
# Using shiny app, you want data in your folder. Not smart, needs direct path
