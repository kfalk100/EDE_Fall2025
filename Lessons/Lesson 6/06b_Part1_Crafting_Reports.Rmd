...
## Advantanges of RMarkdown \# Less \# signs, bigger text
...
-   "Yet another markdown language"  "YAML ain't markdown language"
...
-   The knitting sequence [![image
    link](img/rmarkdownflow.png)](https://env872.github.io/assets/images/rmarkdownflow.png)
-   `Rmd`→[*knitr*]→`md`: # Rmd -\> knitr -\> md -\> pandoc -\>
    PDF/HTML/Word Guided by instructions in YAML & Code chunks
-   `md`→[*Pandoc*]→`PDF`:\
...
-   Global code chunk attributes\
    `knitr::opts_chunk$set(echo = TRUE)` Between { } is chunk attributes
    {r setup} setup is name of chunk
...
## Chunk names & inclusions

Show output only - echo=false Show nothing, just run code -
include=false Nothing when knitting or running Show nothing and do not
run code - Eval=false

## r, fig.height= sets height of figure, fig.width= sets width, fig.cap= is figure caption for entire code chunk. But they are listed figure 1, 2, 3, etc. To have different names, have to be in different chunks

## fig.dim = to set both dimensions at once (fig.dim = c(5,3))

## out.width, out.height scales figures

## fig.align = left, right

## \list of figures

## \newpage

makes new page on knit document Better to have blank white space than
figures that span multiple pages

# is headings

More is smaller \> is block quotes

Ordered and unordered lists 1. Step 1 2. Step 2 3. Step 3 \# Need at
least one space between . and words

-   Bullet 1
-   Bullet 2
-   Bullet 3 \# can use dashes
-   Bullet 1
-   Bullet 2

# Need at least one space between \* and words
