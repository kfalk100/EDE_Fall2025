...
ui <- fluidPage( # Builds the website, can then specify themes, colors, fonts
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      plotOutput("the_scatterplot") # Plot output - plot goes here, server tells you which goes there 
    )))
  ...
  output$the_scatterplot <- renderPlot({
    ...
    ggplot(
      ...
      scale_fill_distiller(
        ...
        guide = "colorbar", 
        direction = 1)
    }) # Does not use plotit function, just render plot
...
# Could call ui something different?
