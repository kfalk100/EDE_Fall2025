...
Determine whether three or more populations have equal means
- Null - all means are equal / equal to total mean
- Alternative - not all means are equal

Reject the null - at least one mean is not equal

Factor - quantity under examination in an experiment as a possible cause of variation in response variable
Level - The categories, measurements, or strata of a factor of interest in the current experiment, also called populations
Balanced designs - an experiment has a balanced design if the factor levels have equal sample sizes

Post Hoc Tests explore differences between multiple group means
- ANOVA results do not identify which particular differences between pairs of means are significant
- TukeyHSD
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) # Do not reject null, P-value is > 0.05
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) # Do not reject null, P-value is > 0.05
...
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) # " " is category
...
> Reorder labels into same as Litter.Totals.groups
...
summary(Litter.anova.2way3) # p-value < 0.05
...
Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass)) # Treatment groups
