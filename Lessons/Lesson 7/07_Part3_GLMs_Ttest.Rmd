...
Test null hypothesis that mean of group is equal to specific value
...
length(EPAair$Ozone) #8976 observations - mean is good representation 

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample #  p-value < 0.05, reject null, population mean is less than 50
...
> This test indicates that the true value of the mean of the ozone levels among our observations is less than 50, due to the rejection of our null hypothesis that it is greater than or equal to 50. The p-value is less than 0.05, leading us to this conclusion, and it agrees with the data summary.
...
- Null hypothesis - 2018 = 2019
- Alternative hypothesis - 2018 does not equal 2019

```{r}
#First let's look at the data
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly() # hard to tell the difference, but seems lower in 2018
...
O3.twosample # p-value <0.05, reject null - that the means are the same / true difference in means between 2018 and 2019 is not equal to 0

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year) # is year a predictor of ozone levels?
summary(O3.twosample2) # p-value < 0.05, year is a predictor of ozone levels
