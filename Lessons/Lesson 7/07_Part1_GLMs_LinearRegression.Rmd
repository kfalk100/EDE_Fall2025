...
Framework for comparing how variables affect different continuous variables

Continuous variable = one or more variables (continuous or categorical) + GLM
...
In hypothesis testing there is:
- Null hypothesis
- Alternative hypothesis

## Steps to Hypothesis Testing
1. State hypotheses and iidentify the claim
2. Find the critical values from appropriate table
3. Compute the test value
4. Make the decision to reject or not reject the null hypothesis
  - P-value < or = to alpha, REJECT
  - P-value > alpha, DO NOT REJECT
- P-values determine whether relationships are statistically significant
- P-value for each independent variable tests the null hypothesis
...
A measure of the % of variability in the values of y that is explained by the independent variable
- Ranges between 0 and 1

## Residuals
- After fitting a regression model, check residual plots first to make sure you have umbiased estimates
...
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top")
theme_set(mytheme) # Set theme for all plots
```
...
irradiance.regression <- 
  lm(PeterPaul.chem.nutrients$irradianceWater ~ # y, dependent variable
       PeterPaul.chem.nutrients$depth) # x, independent variable
...
# Reports residuals, coefficients, standard error, t-value, p-value
# P-value < 0.05, worth trying to estimate irradiance from depth
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) # 2,2 divides into 2 by 2 grid
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #filter dataset, eliminating 0s and outlier to run log (cannot have zero)
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) # new formula, log irradiance by depth, r^2 goes up  to 0.72, explains how much is due to linkage between variables
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black") # se = FALSE removes confidence interval of the line
print(irradiancebydepth2)
...
``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), # Filtering lake
                   tp_ug ~ depth + dissolvedOxygen) # columns
summary(TPregression) # P-value less than 0.05

TPplot <- ggplot(subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                 aes(x = dissolvedOxygen, y = tp_ug, color = depth)) +
  geom_point() +
  xlim(0, 20)
print(TPplot) # some correlation, but depth is also a factor
...
Correlation matrix shows correlation coefficient between variables in data set.
...
corrplot(PeterPaulCorr, method = "ellipse") # Color and shape gives strength and sign of correlation 
corrplot.mixed(PeterPaulCorr, upper = "ellipse") # Mix of upper and ellipse
```
...
#Choose a model by AIC in a Stepwise Algorithm - best model is oxygen, nitrogen, temperature
step(TPAIC)
