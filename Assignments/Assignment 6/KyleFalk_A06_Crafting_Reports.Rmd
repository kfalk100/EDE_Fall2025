...
```{r Plot 1, echo=FALSE, error=FALSE, fig.align='left', fig.cap="2018 Monthly Ozone Levels", fig.height=4, fig.width=7, message=FALSE, warning=FALSE, include=TRUE}
ozone2018 <- epa_data %>%
  filter(Year == 2018)
 

ozone2018plot <- ozone2018 %>% 
  ggplot(aes(
    x = factor(Month),
    y = Ozone)) +
  geom_boxplot() +
...
```{r Plot 2, echo=FALSE, error=FALSE, fig.align='left', fig.cap="2019 Monthly Ozone Levels", fig.height=4, fig.width=7, message=FALSE, warning=FALSE, include=TRUE}
ozone2019 <- epa_data %>% 
  filter(Year == 2019)

ozone2019plot <- ozone2019 %>% 
  ggplot(aes(
    x = factor(Month),
    y = Ozone)) +
  geom_boxplot() +
